#ifndef EBHL_QUERY_WORKLOAD_QUERY_H
#define EBHL_QUERY_WORKLOAD_QUERY_H
#include <mesh.h>
...
namespace polyanya{
    ...
    typedef Workload* WorkloadPtr;

    class Workload_query{
    private:
        const MeshPtr mesh;
    ...
    public:
        ...
        const Workload::grid* start_node;
        const Workload::grid* goal_node;
        double distance;
        ...
        Workload_query() = default;
        Workload_query(MeshPtr m , WorkloadPtr work,  vector<Point> tp, vector<Point> op) :  mesh(m), workload(work), turning_point(tp), obstacle_middle_point(op){ init();}
        Workload_query(Workload_query const &) = delete;
        void operator=(Workload_query const &x) = delete;
        ...
        /*
         * Search function which finds the shortest path of a given st pair
         * Using EHL* to retrieve the location of st and the labels
         */
        bool search(){
            start_node  = workload->get_grid(start);
            goal_node  = workload->get_grid(goal);
            const PointLocation& start_pl = get_point_location_in_search(start, mesh, false);
            const PointLocation& goal_pl = get_point_location_in_search(goal, mesh, false);
            distance = INF_WEIGHT;
            best_hub_id = -1;
            ...
            const unsigned& start_begin = workload->get_grid_labels_begin(start_node->id);
            const unsigned& target_begin = workload->get_grid_labels_begin(goal_node->id);
            for (unsigned i = start_begin, j = target_begin; ; ) {
                int v1 =  workload->get_hub_id(i), v2 = workload->get_hub_id(j);
                if (v1 == numOfVertices) break;  // Sentinel
                //if v1 and v2 are the same hub
                if (v1 == v2) {
                    double lower_bound = workload->get_lower_bound(i) + workload->get_lower_bound(j);
                    ...
                    for(unsigned k = workload->get_convex_begin(i); k != workload->get_convex_end(i);++k){
                        const Convex_vertices_label& start_cv = workload->get_convex_label(k);
                            ...
                            if (tmp_distance < start_distance) {
                                if (start_visibility_times_flag[start_cv.convex_vertex] == visible_search_id) {
                                    if (start_visibility_cache[start_cv.convex_vertex]) {
                                        ...
                                        best_start_predecessor = start_cv.predecessor;
                                        best_start_convex  = start_cv.convex_vertex;
                                        //std::cout << v1 << " " << start_cv.convex_vertex << " " << start_distance << endl;
                                    }
                                } else {
                                    start_visibility_times_flag[start_cv.convex_vertex] = visible_search_id;
                                    start_visibility_cache[start_cv.convex_vertex] = workload->check_visiblity(start,
                                                                                                               obstacle_middle_point[start_cv.convex_vertex],
                                                                                                               turning_point[start_cv.convex_vertex],
                                                                                                               start_cv.convex_vertex
                                    );

//                                            ebhl->check_visiblity(start, start_cv.convex_vertex);
                                    if (start_visibility_cache[start_cv.convex_vertex]) {
                                        //std::cout << v1 << " " << start_cv.convex_vertex << endl;
                                        start_distance = tmp_distance;
                                        best_start_predecessor = start_cv.predecessor;
                                        best_start_convex  = start_cv.convex_vertex;
                                    }
                    ...
                    for(unsigned k = workload->get_convex_begin(j); k != workload->get_convex_end(j);++k){
                        const Convex_vertices_label& target_cv = workload->get_convex_label(k);
                        ...
                        if (target_cv.visibility) {
                            ...
                            if (tmp_distance < target_distance) {
                                if (goal_visibility_times_flag[target_cv.convex_vertex] == visible_search_id) {
                                    ...
                                    goal_visibility_cache[target_cv.convex_vertex] = workload->check_visiblity(goal,
                                                                                                               obstacle_middle_point[target_cv.convex_vertex],
                                                                                                               turning_point[target_cv.convex_vertex],
                                                                                                               target_cv.convex_vertex
                                    );

//                                            ebhl->check_visiblity(goal, target_cv.convex_vertex);
                                    if (goal_visibility_cache[target_cv.convex_vertex]) {
                                        target_distance = tmp_distance;
                                        best_target_predecessor = target_cv.predecessor;
                                        best_target_convex = target_cv.convex_vertex;
...
#endif //EBHL_QUERY_WORKLOAD_QUERY_H
